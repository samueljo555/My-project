...
if (WiFi.status() == WL_CONNECTED) {
  ...
  if (httpCode > 0) {
    ...
    if (daftar == "1" && nama != "" && dispenser=="DIS2"){ // jika ada yang ,mendaftar maka if ini akan di eksekusi
    ...
    lcd.print("Stand By");
        ...
        if(cek!=-1){//jika cek tidak sama dengan -1 maka if ini akan di eksekusi
           ...
           http.begin(address + "/dispenser/database.php?Liter=" + String(TOTAL,2) +"&ID=" + String(cek) + "&DIS=" + "DIS2"));
           http.GET();
           TOTAL=0;
        }
    }    
